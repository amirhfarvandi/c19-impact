...
def interpolate_nm(data, start, end, target_range):
    interpolated_data = {}
...
data = load_data('Data/net-migration.csv')
interpolated_data = interpolate_nm(data, start=2020, end=2025, target_range=range(2021, 2025))

for year, values in interpolated_data.items():
    data[year] = values


write_to_csv(data, 'Interpolations/interpolated-net-migration.csv')