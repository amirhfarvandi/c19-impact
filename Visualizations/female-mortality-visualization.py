...
def compare_original_and_interpolated(data_original, data_interpolated, age_group, start, end):
    ...
    # Customize the plot
    plt.title(f"Comparison of Female Mortality Rates for Age Group {age_group} (Years {start}-{end})", fontsize=16)
...
idata = load_data('Interpolations/female-interpolated-mortality.csv')
odata = load_data('Data/female-mortality.csv')
interpolated_years = range(2021, 2025)
compare_original_and_interpolated(odata, idata, age_group="80+", start=2010, end=2030)
